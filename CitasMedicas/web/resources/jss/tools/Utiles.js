...
//Funcion que devuelve una cadena sin espacios vac�os al inicio y final
...
function TrimLeft(str){
  ...
  //Sale del m�todo si es un valor inv�lido
  ...
  if(str.length == 0)
    ...
    //Recorre la cadena para hallar la posici�n del inicio de la cadena (sin espacios)
...
//LOT 20/06/2008-Inicio Funciones hallar posicion objeto------------------------
  ...
  function fncMove(){
    var posDiv = findPosY(footer); //Se calcula la posicion actual del DIV con id: footer
    ...
    if( posDiv < posFin) //Si la posicion final del DIV es mayor que la actual se cambia (de otro modo apareceria sobre el contenido)
        footer.style.top = posPlus; //Se agrega lo calculado a la posicion 'Y' del  DIV
  }
...
// Este m�todo permite que lleguen al servidor los de los controles que est�n deshabilitados.
